  ...
  if (request.cookies.loggedIn === "true") {
    ...
    try {
      ...
      if (findDonor.rows.length === 0) {
        data.message =
          "Check the inventory page for stocks. You might need to adjust your inventory to match those available by donors";
...
app.get("/test", async (request, response) => {
  ...
  if (request.cookies.loggedIn === "true") {
    ...
    if (recipientTotal + quantity >= 20) {
      // alert('you have exceeded 20 pieces of inventory items. Please find a donor with less quantity to request to stay within your quota for the year');
      data.message =
        "you have exceeded 20 pieces of inventory items. Please find a donor with less quantity to request to stay within your quota for the year";
      response.render("null", { data });
    }
